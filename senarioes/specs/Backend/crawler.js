(function () {
    ...
    var getHtml = function (href, filterFun, downloadFun) {
        ...
        var req = http.get(href, function (res) {
            res.setEncoding(testData.test.encoding1);
    ...
    var downloadImg = function (url) {
        ...
        var req = http.get(reqUrl, function (res) {
            res.setEncoding(testData.test.encoding2);
            res.on('data', function (chunk) {
                data += chunk;
            });

            res.on("end", function () {
                var savePath = testData.test.outForder + narr[0] + narr[1] + narr[2] + "_" + narr[4];
                fs.writeFile(savePath, data, testData.test.encoding2, function (err) {
                    if (err) {
    ...
    describe('Demo', function () {
        ...
        it('Demo - crawler pic', function () {
            maxPage = 1;
            mkdir('-p', testData.test.outForder);
            ...
            var filterFun = function (htmlData) {
                ...
                var resultListData = $(testData.test.filterXpath);
            ...
            for (var i = 1; i <= maxPage; i++) {
                getHtml(testData.test.mailUrl + i, filterFun, downloadImg);
            }
        });
    });//describe end
})();